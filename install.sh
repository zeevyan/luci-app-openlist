...
msg_green "LuCI support for OpenList (Volume Optimized Version)\n"
...
if command -v opkg >/dev/null 2>&1; then
    ...
    # Build-lua outputs 21.02 and 22.03. Defaulting to 22.03 for highest ipk compatibility.
    SDK="openwrt-22.03"
elif [ -x "/usr/bin/apk" ]; then
    msg_red "Error: apk package manager detected."
    msg_red "The lua branch currently only provides .ipk packages for older OpenWrt versions."
    exit 1
else
...
BASE_URL="https://github.com/zeevyan/luci-app-openlist/releases/latest/download/$PKG_FILE"
...
# Remove the '2' from the package names
for pkg in "$TEMP_DIR"/packages_ci/openlist_*.* \
           "$TEMP_DIR"/packages_ci/luci-app-openlist_*.* \
           "$TEMP_DIR"/packages_ci/luci-i18n-openlist-zh-cn_*.*; do
